...
public class KandandaApplication {
    ...
    CommandLineRunner loadData(TournamentLoader loader, MatchResultRepository matchRepo) {
        return args -> {
            ...
            final boolean SHOW_HISTORY = false; // flip true for the full S2-S17 validation trail
            if (SHOW_HISTORY) {
            ...
            if (!wc26.isEmpty()) {
                ...
                for (MatchResult km : wc26) {
                    if (!km.getRound().startsWith("Matchday")) {
                        played.add(km.getHomeTeam().getName() + "|" + km.getAwayTeam().getName());
                        played.add(km.getAwayTeam().getName() + "|" + km.getHomeTeam().getName());
                    }
                }
                for (String[] f : fixtures) {
                    if (played.contains(f[0] + "|" + f[1])) continue; // scored in S19 instead
                    var th = by26.get(f[0]);
                    var ta = by26.get(f[1]);
                ...
                // ----- S19: LIVE scoreboard — every locked call, scored -----
                var liveBt = new com.kandanda.experiment.BacktestService();
                var live = liveBt.scoreLive(wc26, 8.0, -0.1);
                if (live != null) {
                    System.out.println("======== S19 LIVE SCORE (locked calls) ======");
                    for (MatchResult km : wc26) {
                        if (km.getRound().startsWith("Matchday")) continue;
                        var one = liveBt.scoreLiveGame(wc26, km, 8.0, -0.1);
                        System.out.printf("  %-11s %d-%d %-11s (%s)  Brier %.4f%n",
                                km.getHomeTeam().getName(), km.getHomeGoals(), km.getAwayGoals(),
                                km.getAwayTeam().getName(), km.getRound(), one.brier());
                    }
